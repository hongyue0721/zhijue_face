...
| JD 来源 | `InterviewView.jd_source` | `source_type/source_name/is_synthetic` | 不用按钮文案覆盖响应来源 |
| 岗位要求 | `InterviewView.jd_requirements` | `tier/statement/source_span` | 主界面不显示内部 requirement ID |
...
| 开始面试 | `POST /api/v1/interviews/{id}/start` | `expected_revision`、operation | 只有 start operation succeeded 后进入面试页 |
...
| 追问/澄清 | `current_question.kind`、`root_results` | `wording/action/reason_summary/target.followup_intent` | 不展示 observation、rubric、reference points 或模型私有推理 |
...
- `OPERATION_CAPACITY_LIMITED`：提示队列已满；可在同一页面按原业务动作重试。
