...
EXCLUDED_DIRS = {
    ".git", ".venv", "node_modules", "runtime", "dist", "build",
    "__pycache__", ".pytest_cache", ".ruff_cache", ".mypy_cache", ".omp", "v",
}
...
def main() -> int:
    required = ["README.md", "AGENTS.md", "api.md", "process.md", "CHANGELOG.md",
                "config/demo.yaml", "config/environment.env.example",
                "docs/00-owner-guide.md", "docs/14-sources-and-rules.md",
                "ai-prompts/01-start.md", "templates/handoff.md"]
    missing = [p for p in required if not (ROOT / p).is_file()]
    check("关键文件存在", not missing, ", ".join(missing) or f"{len(required)} 项存在")

    text_files = [p for p in iter_assets("*")
                  if p.suffix in {".md", ".json", ".yaml", ".py"}]
    ...
    for path in text_files:
        ...
        if b"\r\n" in raw or "\ufffd" in txt or "\x00" in txt:
            bad_encoding.append(str(path.relative_to(ROOT)))
    check("UTF-8 与 LF", not bad_encoding, ", ".join(bad_encoding) or f"{len(text_files)} 文件检查通过")
    ...
    check("Markdown 本地链接", not broken_links, "; ".join(broken_links) or f"{checked_links} 条路径存在")

    registry = (ROOT / "docs/14-sources-and-rules.md").read_text(encoding="utf-8")
    known_sources = set(re.findall(r"\| (S\d{2}) \|", registry))
    used_sources: set[str] = set()
    for path in markdown_files:
        used_sources |= set(re.findall(r"\[(S\d{2})\]", path.read_text(encoding="utf-8")))
    check("来源编号", used_sources <= known_sources,
    ...
    for node in graph:
        visit(node)
    check("任务依赖无悬空/环", not errors and len(rows) == len(graph),
    ...
    allowed_status = {"PLANNED", "IN_PROGRESS", "IMPLEMENTED", "VERIFIED", "ACCEPTED", "BLOCKED"}
    bad_status = [
        f"{task}:{status}" for task, _, status, _ in rows if status not in allowed_status
    ]
    unsupported = [
        f"{task}:{status}"
    ...
    check("任务状态与证据一致", not bad_status and not unsupported,
          "; ".join(bad_status + unsupported) or f"{len(rows)} 个任务状态合法且完成项均带产物说明")
    ...
    for entry in mapping:
        ...
        check(f"示例: {Path(path).name}", not errors_found,
    ...
    check("负例：未知项不能有等级", rejects(vobs, invalid), "not_assessable + level=3 必须拒绝")
    invalid = load_json("examples/observation-supported.json")
    invalid["confidence"] = 0.94
    check("负例：禁止新增伪置信度", rejects(vobs, invalid), "闭合对象拒绝未声明 confidence")
    invalid = load_json("examples/policy-decision.json")
    invalid["action"] = "CHALLENGE"
    check("负例：策略动作有界", rejects(validators["contracts/policy-decision.schema.json"], invalid), "P0 没有 CHALLENGE 动作")
    invalid = load_json("examples/seed_project_ownership.json")
    invalid["review_status"] = "approved"
    check("负例：未记录审核不能批准", rejects(validators["contracts/seed.schema.json"], invalid), "approved 不允许 null review_record_id")
    ...
    id_map = dict(re.findall(r"^\| `([a-z0-9_]+)` \| (S\d{2}) \|", registry_text, flags=re.M))
    ...
    check("种子引用可追溯", not dangling and seeds_checked > 0,
          "; ".join(dangling) or f"{seeds_checked} 条种子的 reference_id 均映射到已登记来源")
    ...
    for seed_id in (
        ...
        for point in seed["reference_points"]:
            if (
                point["kind"] == "technical"
                and not required_peripheral_sources.issubset(point["reference_ids"])
            ):
    ...
    check("负例：事件 payload 同步动作枚举", rejects(validators["contracts/operation-event.schema.json"], invalid), "拒绝未知事件动作")

    blocks = {b["id"]: b["text"] for b in load_json("examples/source-blocks.json")}
    c = load_json("examples/claim.json")
    good_quotes = all(q["source_block_id"] in blocks and q["exact_quote"] in blocks[q["source_block_id"]]
    ...
    good_quotes = True
    for p in ("examples/observation-supported.json", "examples/observation-unknown.json"):
        for cr in load_json(p)["criteria"]:
            for q in cr["answer_quotes"]:
                good_quotes &= q["answer_id"] in answers and q["exact_quote"] in answers[q["answer_id"]]
    check("Observation 原文定位", good_quotes, "回答引用精确子串存在；不是技术真实性验证")

    cfg = yaml.safe_load((ROOT / "config/demo.yaml").read_text(encoding="utf-8"))
    check("Demo 上限一致", cfg["interview"]["root_question_count"] == 5
    ...
    check("P1 默认未开启", not any(cfg["features"][k] for k in
          ["ocr_enabled", "memory_enabled", "external_web_enabled", "remote_public_access_enabled"]),
          "OCR、跨场记忆、Web、公网均关闭")
    check("本轮示例不冒充审核题库", all(load_json(e["example"])["review_status"] == "draft"
          for e in mapping if e["schema"] == "contracts/seed.schema.json"), "三个题目种子均 draft")

    env = (ROOT / "config/environment.env.example").read_text(encoding="utf-8")
    secret_names = ("API_KEY", "EMBEDDING_API_KEY")
    empty_secrets = all(
        re.search(rf"^{name}=\s*$", env, re.M) for name in secret_names
    )
    check("密钥模板不含真实凭据", empty_secrets
    ...
    lines = ["# 文档包静态校验记录", "", "校验性质：规范资产检查，不是业务软件验收。", "",
    ...
    for name, ok, details in results:
        lines.append(f"| {name} | {'PASS' if ok else 'FAIL'} | {details.replace('|', '/')} |")
    lines += ["", "## 本次没有执行", "",
